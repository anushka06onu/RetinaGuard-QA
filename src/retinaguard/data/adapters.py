...
def parse_eyeq_metadata(
    ...
    label_map = {0: "good", 1: "usable", 2: "reject", "0": "good", "1": "usable", "2": "reject"}
    ...
    for _, r in df_raw.iterrows():
        ...
        canonical_label = label_map.get(raw_label, None)

        w, h, sha = None, None, None
        if img_path.exists():
            try:
...
def parse_deepdrid_metadata(
    ...
    for _, r in df_raw.iterrows():
        img_name = str(r.get("image_id", r.get("image", r.iloc[0])))
        ...
        p_id, eye = extract_patient_and_eye(img_name)
        overall_q = r.get("overall_quality", r.get("quality", None))
        artifact = r.get("artifact", None)
        clarity = r.get("clarity", None)
        field_def = r.get("field_definition", None)
