...
def main():
    parser = argparse.ArgumentParser(description="Train RetinaGuard models with fail-fast validation.")
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument("--fixture-mode", action="store_true", help="Run training using isolated test fixtures in tests/fixtures/")
    args = parser.parse_args()

    print(f"=== Starting Training with Config: {args.config} (Smoke: {args.smoke_test}, Fixture: {args.fixture_mode}) ===")
    results = run_training_experiment(args.config, smoke_test=args.smoke_test, fixture_mode=args.fixture_mode)
